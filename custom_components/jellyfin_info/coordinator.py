...
class JellyfinCoordinator(DataUpdateCoordinator[JellyfinData]):
    ...
    def _api_url(self, endpoint: str) -> str:
        return f"{self.server_url}/{endpoint}?ApiKey={self.auth_token}"
    ...
    def fetch_system(self) -> None:
        try:
            ...
            response = requests.get(self._api_url("System/Info"))
            response.raise_for_status()
    ...
    def get_playing_session(self, username: str) -> Any | None:
        ...
        try:
            ...
            for session in self.data.sessions:
                if (
                    ...
                    _LOGGER.debug(f"Found playing session {session.get("Id")}")
                    return session
            ...
            for session in self.data.sessions:
                if (
                    ...
                    _LOGGER.debug(f"Found paused session {session.get("Id")}")
                    return session
